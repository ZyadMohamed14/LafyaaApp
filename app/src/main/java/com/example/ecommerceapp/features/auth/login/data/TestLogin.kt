...
class FirebaseAuthRepositoryImpl @Inject constructor(
    ...
    ) = login(AppAuthProvider.EMAIL) { auth.signInWithEmailAndPassword(email, password).await() }
    ...
    override suspend fun registerWithFacebook(token: String): Flow<Resource<UserDetailsModel>> {
        return flow {
            try {
                ...
                if (userId == null) {
                    val msg = "Sign up UserID not found"
                    logAuthIssueToCrashlytics(msg, AppAuthProvider.FACEBOOK.name)
                    emit(Resource.Error(Exception(msg)))
                    return@flow
                }
    ...
    override suspend fun registerWithEmailAndPassword(
        ...
        return flow {
            try {
                ...
                if (userId == null) {
                    val msg = "Sign up UserID not found"
                    logAuthIssueToCrashlytics(msg, AppAuthProvider.EMAIL.name)
                    emit(Resource.Error(Exception(msg)))
                    return@flow
                }
        ...
        return flow {
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    emit(Resource.Error(Exception("Error registering user")))
                }
            } catch (e: Exception) {
